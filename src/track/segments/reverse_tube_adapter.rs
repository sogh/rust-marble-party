...
/// A transition segment that morphs from tube (Tube) to flat-with-walls (FlatFloor).
/// This is the reverse of TubeAdapter, providing smooth transitions from tube to flat segments.
pub struct ReverseTubeAdapter {
    /// Length of the adapter segment
    pub length: f32,
    /// Entry tube radius
    pub tube_radius: f32,
    /// Exit width (FlatFloor width)
    pub exit_width: f32,
    /// Wall height at exit
    pub wall_height: f32,
    /// Entry port (Tube profile)
    entry: Port,
    /// Exit port (FlatFloor profile)
    exit: Port,
    /// Bounding box
    bounds: AABB,
    ...
    /// Up vector
    up: Vec3,
    /// Entry floor Y position (bottom of tube)
    entry_floor_y: f32,
}

impl ReverseTubeAdapter {
    pub fn new(length: f32, tube_radius: f32, exit_width: f32, entry_port: Port) -> Self {
        ...
        // Entry floor_y is the bottom of the tube
        let entry_floor_y = match &entry_port.profile {
            PortProfile::Tube { diameter } => entry_port.position.y - diameter / 2.0,
            PortProfile::HalfPipe { diameter } => entry_port.position.y - diameter / 2.0,
            _ => entry_port.position.y - tube_radius,
        };

        // Create entry port with Tube profile
        ...
        // Calculate exit floor Y (descends from entry floor)
        let exit_floor_y = entry_floor_y - descent;

        // Create exit port with FlatFloor profile
        let exit = Port::with_profile(
            exit_pos,
            exit_dir,
            up,
            exit_width / 2.0,
            PortProfile::flat_floor(exit_width, exit_floor_y),
        );

        // Calculate bounds - account for tube at entry and trough at exit
        let wall_height = 1.0; // Default wall height
        let half_exit = exit_width / 2.0;
        let all_points = [
            entry_port.position + right * tube_radius + up * tube_radius,
            entry_port.position - right * tube_radius - up * tube_radius,
            entry_port.position + up * tube_radius,
            entry_port.position - up * tube_radius,
            exit_pos + right * half_exit + up * wall_height,
            exit_pos - right * half_exit,
            exit_pos + up * wall_height,
        ];
    ...
    fn tube_sdf_2d(&self, local_x: f32, local_y: f32, t: f32) -> f32 {
        // Interpolate radius (tube at entry, trough width at exit)
        let radius = Self::lerp(self.tube_radius, self.exit_width / 2.0, t);

        // The tube center transitions from tube center to floor level
        // At t=0: center is at tube_radius above floor (centered tube)
        // At t=1: center is at floor level (y = 0) for trough
        let center_y = Self::lerp(self.tube_radius, 0.0, t);
    ...
    fn trough_sdf_2d(&self, local_x: f32, local_y: f32, t: f32) -> f32 {
        ...
        let half_width = Self::lerp(self.tube_radius, self.exit_width / 2.0, t);
        let wall_h = Self::lerp(self.tube_radius * 2.0, self.wall_height, t);
...
const OVERLAP_ENTRY: f32 = 2.0;
const OVERLAP_EXIT: f32 = 1.0;

impl Segment for ReverseTubeAdapter {
    fn sdf(&self, point: Vec3) -> f32 {
        ...
        // Calculate blend parameter t (0 = entry Tube, 1 = exit FlatFloor)
        ...
        // At t=0: floor is at entry_floor_y (bottom of tube)
        // At t=1: floor is at exit floor level
        let exit_floor_y = match &self.exit.profile {
            PortProfile::FlatFloor { floor_y, .. } => *floor_y,
            _ => self.exit.position.y,
        };
        let floor_y_at_t = Self::lerp(self.entry_floor_y, exit_floor_y, t);
        ...
        // Smooth blend between them (tube at t=0, trough at t=1)
        let blend = Self::smoothstep(t);
        let blended_sdf = tube_sdf * (1.0 - blend) + trough_sdf * blend;

        // Only claim points that are reasonably close
        let max_claim = self.exit_width.max(self.tube_radius * 2.0) * 1.5;
    ...
    fn draw_debug_gizmos(&self, gizmos: &mut Gizmos, color: Color) {
        ...
        for &t in &t_values {
            ...
            let half_width = Self::lerp(self.tube_radius, self.exit_width / 2.0, t);
            let wall_h = Self::lerp(self.tube_radius * 2.0, self.wall_height, t);
            let tube_center_y = Self::lerp(self.tube_radius, 0.0, t);
            ...
            let floor_y = Self::lerp(self.entry_floor_y, exit_floor_y, t);
        ...
        // Draw connecting lines from entry tube to exit trough corners
        let entry_center = self.entry.position;
        let exit_left = exit_pos - self.right * (self.exit_width / 2.0);
        let exit_right = exit_pos + self.right * (self.exit_width / 2.0);

        // Lines showing the transition
        gizmos.line(entry_center - self.right * self.tube_radius, exit_left, color.with_alpha(0.5));
        gizmos.line(entry_center + self.right * self.tube_radius, exit_right, color.with_alpha(0.5));
