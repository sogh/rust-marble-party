...
## Third Request: Dynamic Segment Selection for Track Variety

**Problem:** Tracks were repetitive - selecting a straight tube didn't reduce the chance of the next one being straight.

### Changes Made

Added "repeat penalty" system to `src/track/generator.rs`:

1. **New config fields in `GeneratorConfig`:**
   - `history_depth: usize` (default: 3) - How many recent segments to track
   - `repeat_penalty: f32` (default: 0.25) - Multiplier per occurrence

2. **New field in `TrackGenerator`:**
   - `recent_segments: VecDeque<SegmentType>` - Tracks last N segment types

3. **New methods:**
   - `effective_weight_for()` - Applies penalty: `base_weight * penalty^occurrences`
   - `record_segment()` - Adds segment type to history, maintains max depth

4. **Modified `select_weighted_from()`** - Now uses effective weights instead of base weights

5. **Modified `generate_next()`** - Records segment type after generation

### Example Effect
With default settings (history=3, penalty=0.25):
- Straight tube base weight: 3.0
- After 1 straight in history: 3.0 × 0.25 = 0.75
- After 2 straights in history: 3.0 × 0.0625 = 0.19

**Design decision:** curved_left and curved_right are separate types (left-right-left has no repeats).

---

## Fourth Fix: Keybind Conflict

Changed procedural track generation from 'G' to 'N' (new track) since 'G' was also used for gizmo toggle.
